...
- ``REDACT_PDF``      — opt-in PDF redaction: rasterize each page → redact → re-
  verify → reflatten (Apple Vision, macOS; drops the searchable text layer).
  Off by default → PDFs fail closed.
...
def load(*, rules_path: str | os.PathLike[str] | None = None) -> Config:
    ...
    redact_pdf = os.environ.get(ENV_PDF, "").strip().lower() in _TRUTHY
